#!/usr/bin/env python3
"""
Custom Test command for the MeshCore Bot
Handles the 'test' keyword response with changes to provide snr/rssi only on 0 hop/direct.
"""
...
from modules.utils import calculate_distance, extract_path_node_ids_from_message
...
class TestCommand(BaseCommand):
    ...
    def _is_zero_hop_message(self, message: MeshMessage) -> bool:
        """Return True when message is direct/0-hop.

        Prefers explicit routing metadata and falls back to parsed path text.
        """
        routing_info = getattr(message, 'routing_info', None)
        if routing_info is not None:
        ...
        if getattr(message, 'hops', None) is not None:
            return message.hops == 0

        path_text = (message.path or '').lower()
        return (not path_text) or ('direct' in path_text) or ('0 hops' in path_text)

    def _build_test_connection_info(self, message: MeshMessage, is_zero_hop: bool) -> str:
        """Build connection info for test response.

        Includes SNR/RSSI only for direct (0-hop) messages.
        """
        ...
        if is_zero_hop:
            ...
            return f"{path_part} | {snr_info} | {rssi_info}"
        return path_part

    def _build_direct_signal(self, message: MeshMessage, is_zero_hop: bool) -> str:
        """Build direct-only signal placeholder content.

        Returns SNR/RSSI text for direct (0-hop) messages, otherwise empty string.
        """
        if not is_zero_hop:
            return ""
        snr_info = f"SNR: {message.snr or 'Unknown'} dB"
    ...
    def format_response(self, message: MeshMessage, response_format: str) -> str:
        ...
        try:
            ...
            return response_format.format(
                ...
                timestamp=timestamp,
                elapsed=elapsed,
                snr=(message.snr or self.translate('common.unknown')) if is_zero_hop else "",
                path_distance=path_distance or "",
                firstlast_distance=firstlast_distance or "",
                rssi=(message.rssi or self.translate('common.unknown')) if is_zero_hop else "",
